...
def genetic_algorithm(n_cities, nets, population_size=10000, generations=1000):
    ...
    for generation in range(generations):
        ...
        ranked_population = sorted(population, key=lambda individual: fitnesses[tuple(individual)], reverse=True)
        rank_fitnesses = {tuple(individual): rank for rank, individual in enumerate(ranked_population, start=1)}
        weights = [population_size+1-rank for rank in rank_fitnesses.values()]
        ...
        new_population = []
        for net in nets:
            parent1, parent2 = weighted_random_choice(ranked_population, weights)
...
# Determine the best overall individual and draw the final path
