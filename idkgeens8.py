...
def calculate_fitness(individual, cities):
    ...
    total_distance = sum(calculate_distance(cities[individual[i]], cities[individual[i-1]]) 
...
def draw_path(individual, cities, fitness):
    ...
    # Render the text
    text = font.render(f"Fitness: {fitness}", True, (0, 0, 0))
...
def genetic_algorithm(n_cities, nets, population_size=10000, generations=1000):
    ...
    for generation in range(generations):
        ...
        # Draw the best path every 10 generations
        if generation % 10 == 0:
            draw_path(best_individual, cities, max(fitnesses.values()))
    ...
    # Return the list of best individuals along with the final population and the city locations
...
# Draw the best overall path
best_overall_individual = max(best_individuals, key=lambda individual: calculate_fitness(individual, cities))
draw_path(best_overall_individual, cities, calculate_fitness(best_overall_individual, cities))
